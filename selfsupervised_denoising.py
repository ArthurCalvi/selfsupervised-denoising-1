...
def load_datasets(num_channels, dataset_dir, train_dataset, validation_dataset, prune_dataset=None):
    ...
    if validation_dataset in ['kodak', 'bsd300', 'set14'] + datasets:
        ...
        validation_images = [imageio.imread(x) for x in glob.glob(fn)] #, ignoregamma=True
...
def train(submit_config,
    ...
    #MODIF Arthur
    if real_noise:
        train_images, clean_validation_images, clean_validation_image_size = load_datasets(num_channels, dataset_dir,
                                                                               None if eval_mode else train_dataset+'_clean',
                                                                               validation_dataset+'_clean', prune_dataset)
    ...
    for gpu in range(submit_config.num_gpus):
        with tf.device("/gpu:%d" % gpu):
            ...
            if not real_noise:
                ...
                clean_in_gpu = clean_in_split[gpu]
                _, noise_coeff = noisify(clean_in_gpu, noise_style)
            #MODIF end
